...
async function ensureTargetReachable(target, timeoutMs) {
...
for (const category of categories) {
    ...
    console.log(`${category.padEnd(12)}  ${String(matching.length).padStart(5)}  ${String(vulnerable).padStart(10)}  ${String(inconclusive).padStart(12)}`);
}
if (rows === 0) {
    console.log('(empty)');
}
...
const surfacedFindings = report.findings.filter((f) => f.status === 'vulnerable' || f.status === 'inconclusive');
if (surfacedFindings.length > 0) {
    ...
    const categoriesInDisplayOrder = [
        'dependency',
        'auth',
        'websocket',
        'input',
    ...
    if (cleanupTasks.length === 0) {
        ...
        for (const task of cleanupTasks) {
            ...
            await task.run().catch((err) => {
                console.log(`Cleanup task failed (${task.label}): ${err instanceof Error ? err.message : String(err)}`);
            });
        }
    }
}
